...
eta = 0.05
base_x = simulate_time_series(
    F, a, b, c, sigma, dt, num_steps, x_0=0.0, seed=123
)  # Used as true pdf for sampling initial conditions
initial_conditions = rand_start.randint(
    low=0, high=len(base_x) - 1, size=400
)  # Generate 200 random initial conditions
trajectory = []
unperturbed = []
perturbed = []
...
for i in range(len(differences[0])):  # Loop over time steps
    avg_diff = np.mean(
        [diff[i] for diff in differences]
