...
######## SECOND MOMENT #########
# For reproducibility
rand_start = np.random.RandomState(
    seed=123
...
def simulate_time_series_square(F, a, b, c, sigma, dt, num_steps, x_0, seed):
    ...
    square = []
    x[0] = x_0
    square.append((x[0] - mu) ** 2)
    for i in range(num_steps - 1):
        drift = F + a * x[i] + b * (x[i] ** 2) - c * (x[i] ** 3)
        diffusion = sigma * np.sqrt(dt) * rand_noise.randn()
        x[i + 1] = x[i] + drift * dt + diffusion
        square.append((x[i + 1] - mu) ** 2)
    return x, square


# pertubation magnitude
eta = 0.2
base_x, base_square = simulate_time_series_square(
    F, a, b, c, sigma, dt, num_steps, 0.0, 123
)  # Used as true pdf for sampling initial conditions
initial_conditions = rand_start.randint(
    low=0, high=len(base_x) - 1, size=5000
)  # Generate 400 random initial conditions
trajectory = []
differences = []
...
def simulate_and_compute_diff(initial, eta, dt, num_steps, seed):
    x_init = base_x[
        initial
    ]  # Get the state variable value at the initial condition index'
    _, x_0_2 = simulate_time_series_square(
        F, a, b, c, sigma, dt, num_steps, x_init, seed
    )
    _, x_1_2 = simulate_time_series_square(
        F, a, b, c, sigma, dt, num_steps, eta + x_init, seed
    )
...
rand_seed = np.random.RandomState(
    seed=456
)  # For reproducibility of random seeds for each n
j = 0
for initial in initial_conditions:
    print(j)
    j += 1
    seed_call = rand_seed.randint(
        low=0, high=10000
    )  # Generate a random seed for this initial condition
    diff = simulate_and_compute_diff(
        initial, eta, dt, num_steps_R, seed_call
    )  # Call the function to simulate and compute the difference
    differences.append(diff)  # Store the difference for this initial condition

# get the averages and subtract them
trajectory = [
    np.mean([differences[j][i] for j in range(len(differences))])
    for i in range(num_steps_R)
]  # Compute the average difference at each time step
# window = 150
...
plt.plot(
    time_axis, trajectory, label="Average Squared Trajectory Difference", color="red"
)
plt.title("Dynamical Estimation of Perturbation Effect")
plt.xlabel("Time Steps")
plt.ylabel("Second moment response")
plt.legend()
