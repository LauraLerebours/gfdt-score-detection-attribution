...
def plot_data(
    df: pd.DataFrame, out_path: Optional[str] = None, show: bool = False
) -> None:
...
def _parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        "--noise", type=float, default=0.1, help="std dev of additive Gaussian noise"
    )
    p.add_argument("--seed", type=int, default=None)
    p.add_argument("--out", type=str, default="generated.csv", help="CSV output path")
    p.add_argument(
        "--plot", action="store_true", help="save a plot named generated.png"
    )
