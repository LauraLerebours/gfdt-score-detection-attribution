...
def unnormalized_pdf(x):
    return np.exp(
        (2.0 / sigma**2)
        * (F * x + (a / 2.0) * x**2 + (b / 3.0) * x**3 - (c / 4.0) * x**4)
    )
...
gaussian_pdf = (1.0 / np.sqrt(2 * np.pi * variance)) * np.exp(
    -0.5 * ((x_vals - mu) ** 2 / variance)
)
...
plt.plot(
    x_vals, analytical_pdf, label="Analytical PDF (True)", color="blue", linewidth=2
)

# Plot Gaussian Approximation (Black line)
plt.plot(
    x_vals, gaussian_pdf, label="Gaussian Approximation", color="black", linewidth=1.5
)
...
plt.hist(
    x_sim, bins=100, density=True, color="red", alpha=0.3, label="Empirical / KGMM"
)
