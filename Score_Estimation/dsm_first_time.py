...
class ScoreNet(nn.Module):
    def __init__(self):
        super().__init__()
        self.net = nn.Sequential(
            nn.Linear(1, 256), nn.ReLU(), nn.Tanh(), nn.ReLU(), nn.Linear(256, 1)
        )
...
analytic_score = (2 / (sigma**2)) * (
    F + a * x_range + b * (x_range**2) - c * (x_range**3)
)
...
plt.plot(
    x_range.numpy(), learned_score.numpy(), "r--", label="Learned Score (ScoreNet)"
)
